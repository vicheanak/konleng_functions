...
const storage = new Storage({
  ...
  keyFilename: 'konlengServiceAccount.json',
});
...
const map = maps.createClient({
  key: 'AIzaSyATVsRq1fcn_VVBfWF-9zqOZlyLWmNlJH0',
  Promise: Promise
});
...
const databaseURL = "https://konleng-firebase.firebaseio.com";
...
// admin.initializeApp({
  ...
  const ALGOLIA_ID = functions.config().algolia ? functions.config().algolia.app_id : '4QWVPDJCRN';
  const ALGOLIA_ADMIN_KEY = functions.config().algolia ? functions.config().algolia.api_key : 'a3ed5302a4281f3c8057d83870691d9d';
  const ALGOLIA_SEARCH_KEY = functions.config().algolia ? functions.config().algolia.search_key : 'd778a23e795b491740f373d799b6ddf4';
  ...
  function httpSaveImage(listing, key, callback){
    ...
    .toBuffer();
    Promise
    .all([{width: 1280, height: 800}, {width: 300, height: 300}].map(resize))
            ...
            fileThumb.save(new Buffer(outputBuffer[1].toString('base64'), 'base64'),{
              ...
              if (!err2) {
                fileThumb.getSignedUrl({
                  ...
                  file2 = results[0];
                  callback(null, {large: file1, thumb: file2});
                }).catch((error) => {
  ...
  // Add new listing
  app.post('/listings', getUserByEmail, generateToken, createUser, async (req, res) => {
    ...
    if (!req.body['address']){
      ...
      // let result_type: maps.AddressType = "country | street_address";
      await map.reverseGeocode({
        latlng: map_location
      })
      .asPromise()
      .then((expectOK) => {
        address = expectOK.json.results[0].formatted_address;
      })
      .then((expect2) => {
        res.json(expect2);
      })
    ...
    async.map(req.body['images'], httpGet, (err, body) => {
      ...
      async.mapValues(listings, httpSaveImage, (imgErr, imgUrl) => {
        ...
        if (!imgErr){
          ...
          .createDocumentWithID(db, listingsCollection, listingUuid, listing)
          .then((docRef) => {
            console.log(listingUuid);
            listing['objectID'] = listingUuid;
            const index = client.initIndex(ALGOLIA_INDEX_NAME);
            index.saveObject(listing);
            res.status(200).send(listingUuid);
          })
        }
      });
  ...
  app.post('/import', (req, res) =>{
    firebaseHelper.firestore.restore(db, './listing.json').then(function(success){
  ...
  app.get('/listings', async (req, res) =>{
    ...
    let querySearch: QueryParameters = {
      query: '',
      filters: '',
      attributesToRetrieve: ['address', 'title', 'description', 'province', 'property_type', 'listing_type', 'district', 'price', 'lat', 'lng', 'created_date', 'size', 'images', 'status']
    };
    if (query){
    ...
    let indexSetting: IndexSettings = {
      ...
      'description',
      'address',
      'status', // both attributes have the same priority
